...
class SearchableTest extends TestCase
{
    ...
    public function testScope()
    {
        ...
        $this->assertEquals('select id, title, text FROM (select posts.id, posts.title, posts.text, to_tsvector(\'english\', posts.title) || to_tsvector(\'english\', posts.text) as document from "posts") search where search.document @@ to_tsquery(\'foo\') order by ts_rank(\'foo\') desc', $query->toSql());
    }
